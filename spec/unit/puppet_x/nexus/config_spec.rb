...
describe Nexus::Config do
  ...
  describe :read_config do
    specify 'should raise an error if file is not existing' do
      expect(YAML).to receive(:load_file).and_raise('file not found')
    ...
    specify 'should raise an error if Nexus base url is missing' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.reject{|key,value| key == 'nexus_base_url'})
    ...
    specify 'should raise an error if admin username is missing' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.reject{|key,value| key == 'admin_username'})
    ...
    specify 'should raise an error if admin password is missing' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.reject{|key,value| key == 'admin_password'})
    ...
    specify 'should read Nexus base url' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials)
    ...
    specify 'should raise an error if can_delete_repositories flag is missing' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.reject{|key,value| key == 'can_delete_repositories'})
    ...
    specify 'should read can_delete_repositories flag (false case)' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.merge({'can_delete_repositories' => false}))
      expect(Nexus::Config.read_config[:can_delete_repositories]).to be(false)
    end

    specify 'should read can_delete_repositories flag (true case)' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.merge({'can_delete_repositories' => true}))
      expect(Nexus::Config.read_config[:can_delete_repositories]).to be(true)
    end
    ...
    specify 'should read connection timeout' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.merge({'connection_timeout' => '60'}))
    ...
    specify 'should read connection open timeout' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.merge({'connection_open_timeout' => '60'}))
    ...
    specify 'should read health check retries' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.merge({'health_check_retries' => '100'}))
    ...
    specify 'should read health check timeout' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.merge({'health_check_timeout' => '10'}))
  ...
  describe :can_delete_repositories do
    specify 'should return false if can_delete_repositories configuration parameter is set to false' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials)
      expect(Nexus::Config.can_delete_repositories).to be(false)
    end

    specify 'should return true if can_delete_repositories configuration parameter is set to true' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.merge({'can_delete_repositories' => true}))
      expect(Nexus::Config.can_delete_repositories).to be(true)
    end

    specify 'should return false if can_delete_repositories is some random crap' do
      expect(YAML).to receive(:load_file).and_return(base_url_and_credentials.merge({'can_delete_repositories' => 'We are the champions!'}))
      expect(Nexus::Config.can_delete_repositories).to be(false)
    end
  end
end
