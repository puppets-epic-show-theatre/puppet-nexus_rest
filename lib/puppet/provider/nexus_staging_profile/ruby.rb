...
Puppet::Type.type(:nexus_staging_profile).provide(:ruby) do
  ...
  def self.map_data_to_resource(staging_profile)
    {
        ...
        :promote_notify_roles   => Nexus::Util.a_list_or_absent(staging_profile['promoteNotifyRoles']),
        :promote_notify_creator => Nexus::Util.a_boolean_or_default(staging_profile['promoteNotifyCreator'], :false),
  ...
  def map_resource_to_data
    data = {
        ...
        'promoteNotifyRoles'        => @resource[:promote_notify_roles].split(','),
        'promoteNotifyCreator'      => @resource[:promote_notify_creator] == :true,
